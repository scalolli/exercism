...
class BookStoreTest extends FunSuite with Matchers {
  ...
  test("Group books in the largest most profitable group first") {
    BookStore.groupBooks(List(1, 1, 2, 2, 3, 3, 4, 5)) should be(
      List(
        Group(List(List(1, 2, 3, 4, 5), List(1, 2, 3))),
        Group(List(List(1, 2, 3, 4), List(1, 2, 3, 5))),
        Group(List(List(1, 2, 3), List(1, 2, 3), List(4, 5))),
        Group(List(List(1, 2), List(1, 2), List(3, 4), List(3, 5))),
        Group(
          List(List(1),
               List(1),
               List(2),
  ...
  test("Four groups of four are cheaper than two groups each of five and three") {
    BookStore.total(List(1, 1, 2, 2, 3, 3, 4, 5, 1, 1, 2, 2, 3, 3, 4, 5)) should be(
      10240)
  }
}
